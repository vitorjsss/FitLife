...
type NutricionistHomeNavigationProp = NativeStackNavigationProp<RootStackParamList, "NutricionistHome">;

export default function NutricionistHomeScreen() {
    const [loading, setLoading] = useState(true);
    const [patients, setPatients] = useState<PatientInfo[]>([]);
    const [selectedPatient, setSelectedPatient] = useState<PatientInfo | null>(null);
    const [showConnectModal, setShowConnectModal] = useState(false);
    const navigation = useNavigation<NutricionistHomeNavigationProp>();
    ...
    useFocusEffect(
        useCallback(() => {
            if (user?.id) {
                ...
                PatientProfessionalAssociationService.getPatientsByNutricionistId(user.id)
                    .then(patientsList => {
                    ...
                    .catch(error => console.error('[NutricionistHome] Erro ao recarregar:', error));
            }
        }, [user?.id])
    );
    ...
    const loadPatients = async () => {
        ...
        try {
            setLoading(true);
            const patientsList = await PatientProfessionalAssociationService.getPatientsByNutricionistId(user.id);
            setPatients(patientsList);
            ...
            console.error('[NutricionistHome] Erro ao carregar pacientes:', error);
    ...
    return (
        <View style={styles.container}>
            <Header title="Nutricionista - Início" showBackArrow={false} showUserIcon={true} />
                ...
                <View style={styles.patientsSection}>
                    <View style={styles.sectionHeader}>
                        <Text style={styles.sectionTitle}>
                            <Icon name="users" size={20} color="#1976D2" /> Meus Pacientes
                        </Text>
                        <TouchableOpacity
                    ...
                    {patients.length === 0 ? (
                        <View style={styles.emptyState}>
                            ...
                            <Text style={styles.emptyStateText}>Nenhum paciente associado</Text>
                        </View>
                    ) : (
                        <FlatList
                            data={patients}
                    ...
                    <View style={styles.actionsSection}>
                        ...
                        <View style={styles.actionsGrid}>
                            <TouchableOpacity
                                ...
                                onPress={() => navigation.navigate('Refeicoes', { patientId: selectedPatient.patient_id })}
                            >
                                <Icon name="cutlery" size={32} color="#fff" />
                                <Text style={styles.actionCardText}>Refeições</Text>
                            </TouchableOpacity>

                            <TouchableOpacity
                                style={styles.actionCard}
                                onPress={() => navigation.navigate('CalendarioRefeicoes', { patientId: selectedPatient.patient_id })}
                            >
