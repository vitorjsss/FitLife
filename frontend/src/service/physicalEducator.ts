...
class PhysicalEducatorService {
    ...
    async getById(id: number): Promise<any> {
        const endpoint = API_CONFIG.ENDPOINTS.PHYSICAL_EDUCATOR.GET_BY_ID.replace('{id}', id.toString());
        const response = await apiClient.get(endpoint);
        return response.data;
    }

    async update(id: number, data: Partial<PhysicalEducatorData>): Promise<any> {
        const endpoint = API_CONFIG.ENDPOINTS.PHYSICAL_EDUCATOR.UPDATE.replace('{id}', id.toString());
